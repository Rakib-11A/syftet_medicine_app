...
module Api
  module V1
    class ProductsController < Api::ApiBase
      ...
      def show
        ...
        product.variants.each do |varient|
          response[:varients] << {
            info: varient.as_json(only: %i[id sku product_id color color_image size]),
            images: []
          }
      ...
      def filters
        ...
        render json: {
          categories: Admin::Category.where('parent_id IS NULL').as_json(only: %i[id name]),
