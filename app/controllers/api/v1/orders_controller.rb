...
module Api
  module V1
    class OrdersController < Api::ApiBase
      ...
      def populate
        ...
        variant = Product.find(params[:product_id])
      ...
      def update_address
        ...
        if order.present? && !order.completed?
          if order.ship_address.present?
            unless order.ship_address.update(ship_address_params)
      ...
      def ship_address_params
        { firstname: params[:ship_address][:firstname], last_name: params[:ship_address][:lastname],
          address: params[:ship_address][:address], city: params[:ship_address][:city], zipcode: params[:ship_address][:zipcode], phone: params[:ship_address][:phone], state: params[:ship_address][:state], country: params[:ship_address][:country] }
      end
      ...
      def find_cart_by_token_or_user
        if request.headers['GuestToken'].present?
          @token = request.headers['GuestToken']
      ...
      def find_order_by_token_or_user(options = {}, with_adjustments = false)
        ...
        incomplete_orders = Order.incomplete.includes(line_items: [variant: %i[images option_values product]])
