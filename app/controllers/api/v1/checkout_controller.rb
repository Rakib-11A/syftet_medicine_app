...
module Api
  module V1
  class CheckoutController < Api::ApiBase
    ...
    def express_payment
      PaypalExpressCheckout.create({ payer_id: params[:payer_id] }),
    ...
    def set_state_if_present
      return unless @order.present? && params[:state]
    ...
    def associate_user
      return unless @current_user && @order.present?
    ...
    def check_authorization
      return unless @current_user.present? && @order.present?
    ...
    def add_store_credit_payments
      return unless params.key?(:apply_store_credit) && @order.present?
      ...
      return unless @order.payments.valid.sum(:amount) < @order.net_total
