...
module Admin
  class UsersController < BaseController
    before_action :set_user, only: %i[show edit update orders addresses]
    ...
    def update
      ...
      flash.now[:success] = t(:account_updated) if @user.update(user_params)
    ...
    def user_params
      params.require(:user).permit(:name, :email, :password, :password_confirmation, :role, :company, :initial_balance,
                                   :initial_balance_date, :mobile, :landphone)
    end
    ...
    def sign_in_if_change_own_password
      return unless current_user == @user && @user.password.present?
    ...
    def collection(users)
      ...
      if params[:quick_search].present?
        ...
        unless params[:user][:email].blank?
          users = users.where("lower(email) like '%#{params[:user][:email].downcase}%'")
        end
        users
      else
        users
