  ...
  module Suppliers
    class PurchaseOrdersController < BaseController
      ...
      before_action :set_purchase,
                    only: %i[cart return_item refund new_refund_items received_purchase update_receive_item complete update_purchase
                             update_item add_item remove_item show edit update destroy receive refund]
      ...
      def refunds
        ...
        @refunds = Admin::Suppliers::Refund.search(params[:q], @refunds) if params[:q].present?
      ...
      def complete
        @supplier = User.suppliers.find_by(id: @purchase.supplier_id)
        @stock_locations = StockLocation.all
        return if @purchase.items.exists?
      ...
      def add_item
        variant = Product.find_by(id: params[:variant_id]) if params[:variant_id].present?
      ...
      def update_item
        ...
        if purchase_item && quantity != purchase_item.issued_quantity
          if quantity > purchase_item.issued_quantity
            ...
            purchase_item.save!
            p purchase_item
          elsif quantity < purchase_item.issued_quantity && quantity.positive?
            purchase_item.issued_quantity = quantity
      ...
      def update_receive_item
        ...
        sale_price = params[:sale_price].to_f
        vat = params[:vat].to_f
        purchase_quantity = purchase_item.received_quantity || purchase_item.issued_quantity
        if purchase_item && quantity != purchase_quantity
          ...
          elsif quantity < purchase_quantity && quantity.positive?
            purchase_item.decrease_receive_quantity(quantity)
          else
            unless quantity.present?
      ...
      def return_item
        ...
        if @refund.present?
          ...
          @refund = @purchase.refunds.create!(supplier: @purchase.supplier, invoice_no: @purchase.no, amount: 0,
                                              date: Date.today, refund_by: current_user.name, refund_reason: params[:reason], is_order: true)
        end
        @invoice_item = @purchase.items.find(params[:item_id])
        p @invoice_item
        quantity = params[:quantity].present? ? params[:quantity].to_i : 1
        @refund_item = @refund.items.create!(invoice_item_id: @invoice_item.id,
                                             amount: @invoice_item.cost_price * quantity, quantity: quantity)
