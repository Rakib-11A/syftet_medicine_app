...
module Admin
  module Suppliers
    class PaymentsController < BaseController
      before_action :set_admin_suppliers_payment, only: %i[show edit update destroy]
      ...
      def complete
        ...
        @payments = Admin::Suppliers::Payment.where('supplier_id = ? AND (payment_date = ? OR value_date = ?)',
                                                    payment.supplier_id, date, date)
        @payments.each(&:supplier_payment_complete)
      ...
      def admin_suppliers_payment_params
        params.require(:admin_suppliers_payment).permit(:supplier_id, :paid_by_id, :method, :amount, :payment_date,
                                                        :value_date, :cheque_number, :status, :confirmed, :paid_to, :invoice_id, :bank_name, :bank_branch, :is_group_payment, :commission, :bank_account_id)
      end
    end
  end
end
