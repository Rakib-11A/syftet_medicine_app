...
module Admin
  module Suppliers
    class RefundItem < ApplicationRecord
      ...
      def update_stock
        return unless invoice_item.present? || product.present?

        create_products_stock(product_id: product.present? ? product.id : invoice_item.product_id,
                              quantity: (quantity * -1), stock_location_id: refund.invoice.present? ? refund.invoice.stock_location_id : '')
      end
    end
  end
end
