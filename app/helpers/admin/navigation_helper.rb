...
module Admin
  module NavigationHelper
    ...
    def tab(*args)
      ...
      titleized_label = t(options[:label], default: options[:label], scope: %i[admin tab]).titleize
    ...
    def klass_for(name)
      ...
      [model_name.classify.to_s, model_name.classify,
       model_name.gsub('_', '/').classify].find(&:safe_constantize).try(:safe_constantize)
    end
    ...
    def link_to_with_icon(icon_name, text, url, options = {})
      options[:class] = "#{options[:class]} icon-link with-tip".strip
      options[:class] += ' no-text' if options[:no_text]
      options[:title] = text if options[:no_text]
      text = options[:no_text] ? '' : content_tag(:span, text.to_s, class: 'text')
    ...
    def main_part_classes
      return 'col-sm-12 col-md-12 sidebar-collapsed' if cookies['sidebar-minimized'] == 'true'

      'col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2'
    end

    def wrapper_classes
      return unless cookies['sidebar-minimized'] == 'true'
