...
module Admin
  module BaseHelper
    def flash_alert(flash)
      ...
      flash_div = content_tag(:div, close_button + message,
                              class: "alert alert-#{flash_class} alert-auto-disappear")
    ...
    def link_to_icon_remove_fields(f)
      ...
      link_to_with_icon('delete', '', url, class: 'spree_remove_fields btn btn-sm btn-danger',
                                           data: { action: 'remove' }, title: t(:remove)) + f.hidden_field(:_destroy)
    end

    def preference_fields(object, form)
      # p object.class::PREFERENCES
      # return unless object.class.respond_to?(:PREFERENCES)
      fields = object.class::PREFERENCES.map do |preference|
